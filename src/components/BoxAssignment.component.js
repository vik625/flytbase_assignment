...
class BoxAssignment extends Component {
    ...
    deleteBox(id) {
        ...
        for(var i = 0; i < temp.length; i++) {
            ...
            if(id != temp[i].props.id) {
                this.setState({boxes: [...this.state.boxes, <MovingBox id = {boxId} setTarget = {(bId) => {this.setTarget(bId)}}/>]});
            }
        }
    }
    ...
    setTarget(id) {
        ...
        window.addEventListener("keydown", (event) => {
            ...
            if(this.state.currentTarget == id) {
                switch(event.key) {
                    case "w": if(box.offsetTop >= 10) box.style.top = parseInt(box.style.top) - (movePixels)+"px"; break;
                    case "s": if(bottomBound >= 10) box.style.top = parseInt(box.style.top) + (movePixels)+"px"; break;
                    case "a": if(box.offsetLeft >= 10) box.style.left = parseInt(box.style.left) - (movePixels)+"px"; break;
                    case "d": if(rightBound >= 10) box.style.left = parseInt(box.style.left) + (movePixels)+"px"; break;
                    case "Delete": this.deleteBox(id)
                }
            } else {
                if(movePixels == 5) {
                    movePixels = 0;
    ...
    toggleAllListeners() {
        ...
        if(this.state.blockEvent) {
            this.setState({blockEvent: false, toggleImage: "https://img.icons8.com/color/48/000000/toggle-on.png"})
        } else {
            this.setState({blockEvent: true, toggleImage: "https://img.icons8.com/color/48/000000/toggle-off.png"})
        }
    ...
    addBox() {
        ...
        this.setState({boxes: [...this.state.boxes, <MovingBox id = {newBoxId} setTarget = {() => {this.setTarget(newBoxId)}} deleteBox = {() => {this.deleteBox(newBoxId)}}/>], currentBoxId: newBoxId});
    }

    render() {
        return(
            //Boundary for containing all the boxes
            <div id = "boundary" className = "container card" style = {{width: "500px", maxWidth: "550px", minHeight: "400px", ht: "400"}}>
                {/* Button for adding more boxes */}
                <h3 onClick = {this.addBox} className = "btn btn-primary" style = {{margin: "10px auto 10px auto"}}>
                    Add Box
                </h3>

                {/* Toggle button for blocking keyboard operations (event listeners) */}
                <center style = {{position: "absolute", top: "0px", right: "20px", width: "60px"}}>
                    <img onClick = {this.toggleAllListeners} src = {this.state.toggleImage} style = {{width: "50px", height: "50px", marginBottom: "-10px"}}/>
                    <font size = "2"><b>Keyboard</b></font>
                </center>
